...
final class RestEventSource implements EventSourceInterface
{
    ...
    public function fetchEvents(int $lastEventId, int $limit = 1000): array
    {
        try {
            ...
            throw new SourceUnavailableException(
                sprintf('REST source "%s" unavailable: %s', $this->name, $e->getMessage()),
                previous: $e
            );
        }
    }
    ...
    private function mapToDTO(): EventDataDTO
    {
        return new EventDataDTO(
            id: 'error',//this is error which will not break the loop
